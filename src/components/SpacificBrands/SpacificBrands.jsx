...
export default function SpacificBrands() {
            ...
            <div className="container 2xl:mx-auto">
              <div className="row">
                <div className="flex flex-wrap justify-center items-center">
                  {products.map((brands) => (
                    <div key={brands.id} className="px-2 pb-3 mb-2">
                      <div className="w-full max-w-80 sm:max-w-60 bg-white border rounded-lg shadow dark:bg-[#647A67] dark:border-[#1F241F] hover:scale-105 transition-all duration-500">
                        <Link to={`/productdetails/${brands.id}/${brand}`}>
                          <div className="p-5">
                            <img
                        ...
                        <div className="px-5 pb-5">
                          ...
                          >
                            <Link
                              to={`/productdetails/${brands.id}/${brands.category?.name}`}
                            >
                              <h5 className="text-xl font-[Roboto-Bold] tracking-tight text-gray-900 dark:text-white">
                                {brands.title.split("").slice(0, 14).join("")}
                              </h5>
                            ...
                            <button onClick={() => toggleWishlist(brands?.id)}>
                            <FontAwesomeIcon
                          ...
                          <div className="flex items-center mt-2.5 mb-5">
                            <div className="flex items-center space-x-1 rtl:space-x-reverse">
                              {getStarRate(brands.ratingsAverage)}
                            </div>
                          ...
                          <div className="flex items-center justify-between">
                            <span className="font-[Roboto-Bold] max-text-3xl font-bold text-gray-900 dark:text-white">
                              ...
                              {brands.price.toLocaleString()}
                              <span className="text-gray-300 text-xs md:text-sm">
                                {brands.price % 10 === 9 ? ".99" : ".00"}
                              </span>
                            </span>
                            <button
                              onClick={() => addToCart(brands?.id)}
